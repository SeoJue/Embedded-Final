...
습도 감지 쓰레드, 거리 감지 쓰레드, 블루투스 쓰레드 등을 실행 
...
## 문제점
### 1. 온습도 센서 문제  
> 무슨 이유에서인지 온습도 센서는 다른 멀티 쓰레드 환경에서  
...
## 해결방안
### 1. 온습도 센서 문제 해결
> 시스템 구조상 기존에는 Lock을 사용할 필요가 없었으나  
> 온습도 센서의 정상적 수행을 위해 메인 쓰레드를 제외한 다른 쓰레드들이  
> 동시에 자신의 기능 함수를 수행할 수 없도록 할 필요가 생겼기  LOCK을 추가하였고 문제를 해결했다. 
